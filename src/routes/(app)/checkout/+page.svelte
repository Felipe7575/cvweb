...
<div class="container mx-auto">
    ...
    <div class="card bg-base-100 mx-auto w-full shadow-xl md:w-fit md:p-6 p-3 ">
        <h2 class="text-2xl font-semibold text-gray-800 text-center">{$t("buy_credits")}</h2>

        <div class="divider"></div>

        <!-- DISPLAY CURRENT CREDIT BALANCE -->
        <div class="flex mb-4 text-center">
            <p class="text-lg text-gray-600">{$t("current_balance")}:</p>
            <p class="text-lg px-3 ">{userCredits} {$t("credits")}</p>
        </div>

        <!-- CREDIT INPUT -->
        <div class="mb-4">
            <label class="text-lg text-gray-600">{$t("credits_to_buy")}:</label>
            <input 
                type="number" 
        ...
        <p class="text-lg text-gray-600">
            <span class="font-semibold">{$t("total_price")}:</span>
        ...
        <button 
            ...
            disabled={loading}
        >
            {loading ? $t("processing") : $t("buy_button") }
        </button>
    ...
    <div class="modal modal-open">
        <div class="modal-box">
            <div class="flex flex-row justify-between items-center">
                <h3 class="text-lg font-semibold pl-3">{$t("redeem_coupon_modal_title")}</h3>
                <!-- CLOSE BUTTON -->
                <div>
            ...
            {#if formReedemCoupon}
                ...
                >
                    ...
                    />
                    <button type="submit" class="btn btn-primary w-full">{$t("redeem_button")}</button>
                </form>

                {#if $messageReedemCoupon?.error}
                    <p class="text-red-500 mt-2 text-center">{$t("coupon_error_message")}</p>
                {/if}

                {#if $messageReedemCoupon?.creditsAdded}
                    <p class="text-green-500 mt-2 text-center">{$t("credits_added_message")+ " " + $messageReedemCoupon.creditsAdded} </p>
                {/if}
